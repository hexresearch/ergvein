...
instance (Reflex t, LocalizedPrint (InplaceEditLbl lbl)) => Default (InplaceEditCfg t (InplaceEditLbl lbl)) where
  def = InplaceEditCfg {
...
inplaceEditField :: forall t m a lbl . (DomBuilder t m, PostBuild t m, MonadFix m, MonadHold t m, MonadLocalized t m, LocalizedPrint (InplaceEditLbl lbl), LocalizedPrint lbl)
...
inplaceEditField InplaceEditCfg{..} renderVal parseVal val0D = mdo
  ...
  valD <- mergeDyn val0D $ fforMaybe actE $ \case
    ...
    _ -> Nothing
  actD <- networkHoldDyn $ ffor tglD $ \case
  ...
  where
    ...
    editWidget valD = el "div" $ do
      ...
      (goE, delE, closeE) <- divClassDyn _inplaceBtnGroup $ do
        ...
        delE <- fmap switchDyn $ networkHoldDyn $ ffor _inplaceCanDelete $ \b ->
      ...
      _ <- networkHoldDyn $ ffor _inplaceSeparator $ \msep -> case msep of
        Nothing -> pure ()
    ...
    validateVal txtE = do
      ...
      void $ networkHold (pure ()) $ ffor valE $ \case
        Left er -> divClassDyn _inplaceErrorClass $ localizedText er
        _ -> pure ()
      pure $ fmapMaybe justRight valE
