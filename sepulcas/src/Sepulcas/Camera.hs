...
openCamara :: (PerformUI t m, PlatformNatives) => Event t () -> m (Event t ())
openCamara e = runOnUiThread $ ffor e $ \_ -> do
  cameraWork ""
  pure ()

getResultCamara :: (PerformUI t m, PlatformNatives) => Event t () -> m (Event t Text)
getResultCamara e = runOnUiThread $ ffor e $ const cameraGetResult

waiterResultCamera :: (PerformUI t m, PlatformNatives, MonadFix m) => Event t () -> m (Event t Text)
...
debugCameraPage :: (DomBuilder t m, PerformUI t m, MonadLocalized t m, PostBuild t m, MonadHold t m, PlatformNatives, MonadFix m) => m ()
debugCameraPage = do
