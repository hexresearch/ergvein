...
indexerNodeController :: (MonadHasMain m, MonadClient t m, MonadSettings t m) => [ErgveinNodeAddr] -> m ()
...
connectionWidget :: MonadClient t m => IndexerConnection t -> m (Event t ())
