...
data PasswordWidgetStrings = PWSPassword | PWSPassNamed Text | PWSRepeat | PWSSet | PWSNoMatch | PWSGo | PWSLogin | PWSEmptyLogin | PWSEmptyPassword | PWSEmptyPattern | PWSDeriv | PWSInvalidPath
...
instance LocalizedPrint PasswordWidgetStrings where
  localizedShow l v = case l of
    English -> case v of
      ...
      PWSEmptyPattern  -> ""
      PWSDeriv         -> "You can override suggested derivation path. Leave it untouched if not sure what it is."
    ...
    Russian -> case v of
      ...
      PWSEmptyPattern  -> ""
      PWSDeriv         -> "Вы можете переназначить путь вывода ключей. Оставьте как есть, если не знаете, что это такое."
