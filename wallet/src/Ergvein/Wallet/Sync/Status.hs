...
instance LocalizedPrint SyncProgress where
  localizedShow l SyncMeta{syncMetaStage = syncMetaStage@SyncFilters, ..} = case l of
    English -> "Sync " <> localizedShow l syncMetaStage <> " of " <> showt syncMetaCur <> "\n" <> showt (percent syncMetaAmount syncMetaTotal) <> "%"
    Russian -> "Синхр " <> localizedShow l syncMetaStage <> " " <> showt syncMetaCur <> "\n" <> showt (percent syncMetaAmount syncMetaTotal) <> "%"

  localizedShow l SyncMeta{syncMetaStage = syncMetaStage@(SyncAddressInternal _), ..} = case l of
    English -> "Sync " <> showt syncMetaCur <> " change address " <> localizedShow l syncMetaStage <> "\n" <> showt (percent syncMetaAmount syncMetaTotal) <> "%"
    Russian -> "Синх " <> showt syncMetaCur <> " адреса для сдачи " <> localizedShow l syncMetaStage <> "\n" <> showt (percent syncMetaAmount syncMetaTotal) <> "%"

  localizedShow l SyncMeta{syncMetaStage = syncMetaStage@(SyncAddressExternal _), ..} = case l of
    English -> "Synс " <> showt syncMetaCur <> " receiving address " <> localizedShow l syncMetaStage <> "\n" <> showt (percent syncMetaAmount syncMetaTotal) <> "%"
    Russian -> "Синхр " <> showt syncMetaCur <> " адреса для получения " <> localizedShow l syncMetaStage <> "\n" <> showt (percent syncMetaAmount syncMetaTotal) <> "%"

  localizedShow l SyncMeta{syncMetaStage = syncMetaStage@(SyncBlocks _ _), ..} = if isAndroid
    then case l of
      English -> "Synс new blocks of " <> showt syncMetaCur <> "\n" <> showt (percent syncMetaAmount syncMetaTotal) <> "%"
      Russian -> "Синхр новых блоков " <> showt syncMetaCur <> "\n" <> showt (percent syncMetaAmount syncMetaTotal) <> "%"
     else case l of
