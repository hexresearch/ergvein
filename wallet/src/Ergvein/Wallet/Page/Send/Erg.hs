...
sendWidget mInit title navbar thisWidget = wrapperNavbar False title thisWidget navbar $ divClass "send-page" $ mdo
  ...
  retInfoD <- formClass "mb-0" $ mdo
    recipientD <- recipientWidget ERGO recipientInit submitE
    amountD <- sendAmountWidgetErg amountInit submitE
    feeD <- feeSelectionWidgetErg (FSRate ERGO) feeInit submitE
