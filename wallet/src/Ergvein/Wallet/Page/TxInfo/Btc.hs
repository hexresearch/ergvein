...
module Ergvein.Wallet.Page.TxInfo.Btc(
    txInfoPage
  , showTime
  ) where
...
txInfoPage txView@TxView{..} = do
  ...
  wrapper False title thisWidget $ divClass "tx-info-page" $ do
    ...
    infoPageElementEl HistoryTIRbf $ do
      text $ showt $ txDetailedView'rbfEnabled txView'detailedView
      let bumpFeePossible = txView'inOut == TransWithdraw && txDetailedView'rbfEnabled txView'detailedView && txDetailedView'confirmations txView'detailedView == 0
      when bumpFeePossible $ do
        bumpFeeE <- divClass "mt-1" $ outlineButton HistoryTIBumpFeeBtn
        void $ nextWidget $ ffor bumpFeeE $ const
          Retractable
            {retractableNext = bumpFeePage currency txView Nothing,
