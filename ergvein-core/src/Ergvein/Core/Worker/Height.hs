...
heightAsking :: (MonadWallet t m, MonadStatus t m, MonadNode t m) => m ()
heightAsking = void . networkHoldDyn . fmap (traverse_ heightAsker) =<< getActiveCursD

heightAsker :: (MonadWallet t m, MonadStatus t m, MonadNode t m) => Currency -> m ()
...
heightAskerBtc :: (MonadWallet t m, MonadStatus t m, MonadNode t m) => m ()
heightAskerBtc = do
...
startBtcFlow :: (MonadWallet t m, MonadStatus t m, MonadNode t m) => Workflow t m ()
...
btcCatchUpFlow :: (MonadWallet t m, MonadStatus t m, MonadNode t m) => Word64 -> (Timestamp, [(BlockHeight, BlockHash)]) -> Workflow t m ()
...
btcListenFlow :: (MonadWallet t m, MonadNode t m, MonadStatus t m) => BlockHash -> Timestamp -> BlockHeight -> Workflow t m ()
